...
from tests.fixtures import graph, groups, permissions, session, users  # noqa: F401
from tests.util import add_member, edit_member, grant_permission, revoke_member
...
def expiring_graph(session, graph, users, groups, permissions):  # noqa: F811
    ...
    add_member(
        groups["team-sre"], users["zebu@a.co"], role="owner", expiration=note_exp_now + week
    )
